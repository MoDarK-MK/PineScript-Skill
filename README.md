...
- 🔍 **63-Rule Linter** — v6 compile errors, repainting traps, scope violations, performance costs, invisible dashboard text. Seven rules repair themselves with `--fix`
...
- 🧬 **Tests That Are Themselves Tested** — `mutate_check.py` disables each lint rule and confirms the suite goes red. Its first run found 16 of 63 rules with no test at all
...
| Guide | Purpose |
|-------|---------|
| **[volume-keylevels-guide.md](references/volume-keylevels-guide.md)** | Volume & Key Levels: Auction Market Theory, Volume Profile, vPOCs, CVD, Anchored VWAP, and clustering |
...
| | |
|---|---|
| **63 lint rules** | PINE001–PINE064 (PINE024 vacant), fact-checked against TradingView's docs; 7 auto-fixable |
...
| **Mutation-checked** | 63/63 rules verified to have a test that fails when the rule is disabled |
